...
async def async_setup_entry(
    ...
    for camera in cameras:
        ...
        try:
            ...
            _LOGGER.warning(
                f"Error fetching WebRTC session configuration for camera {camera._attr_name}: {e}"
            )
...
class WyzeCamera(CameraEntity):
    ...
    async def config_fetch(self) -> None:
        ...
        _LOGGER.debug(
            f"Initial fetch of WebRTC session configuration complete for camera {self._attr_name}"
        )
    ...
    def _async_get_webrtc_client_configuration(self) -> WebRTCClientConfiguration:
        ...
        _LOGGER.debug(
            f"WebRTC session configuration for camera {self._attr_name}: {config}"
        )
        ice_servers = []
    ...
    async def async_handle_async_webrtc_offer(
        ...
        _LOGGER.debug(
            f"Handling WebRTC offer for camera {self._attr_name} with session ID {session_id}"
        )
...
class WyzeCameraWebRTCSession:
    ...
    async def send_offer(self, offer_sdp: str):
        ...
        _LOGGER.debug(
            f"Sending SDP offer for camera {self.camera._attr_name} with session ID {self.session_id}, {str_payload}"
        )
    ...
    async def send_candidate(self, candidate: RTCIceCandidateInit):
        ...
        str_payload = json.dumps(payload)
        _LOGGER.debug(
            f"Sending ICE candidate for camera {self.camera._attr_name} with session ID {self.session_id}: {str_payload}"
        )
        await self.websocket.send(str_payload)
    ...
    async def run_loop(self):
        ...
        self.close = close
        _LOGGER.debug(
            f"run_loop starting for camera {self.camera._attr_name} session {self.session_id}"
        )
        try:
            async for message in self.websocket:
                if len(message) == 0:
                    _LOGGER.debug(
                        f"Received empty message (type={type(message).__name__}) for camera {self.camera._attr_name} session {self.session_id}"
                    )
                    continue
                _LOGGER.debug(
                    f"Received message for camera {self.camera._attr_name} with session ID {self.session_id}: {message}"
                )
                try:
                    data = json.loads(message)
                except json.JSONDecodeError as e:
                    _LOGGER.error(
                        f"Failed to decode JSON message for camera {self.camera._attr_name} with session ID {self.session_id}: {e}"
                    )
                    continue
                match data.get("messageType"):
                    ...
                    case "STATUS_RESPONSE" | "GO_AWAY" | "RECONNECT_ICE_SERVER":
                        _LOGGER.debug(
                            f"KVS control message '{data.get('messageType')}' for session {self.session_id}: {data}"
                        )
                    case other:
                        _LOGGER.debug(
                            f"Unhandled KVS message type '{other}' for session {self.session_id}: {data}"
                        )
        except Exception as e:
            _LOGGER.error(
                f"run_loop error for camera {self.camera._attr_name} session {self.session_id}: {e}",
                exc_info=True,
            )
        _LOGGER.debug(
            f"run_loop exited for camera {self.camera._attr_name} session {self.session_id}"
        )
