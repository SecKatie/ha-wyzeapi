...
def pack_l2_lock_unlock(ble_id: int, ble_token: str, challenge: bytes, command):
    ...
    encrypted_challenge = b''.join((x^y).to_bytes(1) for x,y in zip(encrypted_challenge, magic_bytes))
...
def crc(data):
    magic = '0000c0c1c1810140c30103c00280c241c60106c00780c7410500c5c1c4810440' \
    ...
    magic = [int.from_bytes(binascii.unhexlify(magic[x:x+4])) for x in range(0, len(magic), 4)]
    result = 0
    for b in data:
