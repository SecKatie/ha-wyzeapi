...
from .wyzeapi_exceptions import AccessTokenError
...
class WyzeApi:
    def __init__(self, user_name, password):
        ...
        self.device_id = "bc151f39-787b-4871-be27-5a20fd0a1937"
        self.__in_error_state = False
        self.__invalid_access_tokens = []
    ...
    async def async_get_devices(self):
        ...
        if not self.__all_devices:
            ...
            data = await self.async_do_request(url, payload)
            self.__all_devices = data['data']['device_list']
    ...
    async def async_list_contact_sensor(self):
        ...
        for device in await self.async_get_devices():
            if device['product_type'] == "ContactSensor":
                contact_sensors.append(WyzeContactSensor(
                    self,
                    device['mac'],
                    device['nickname'],
    ...
    async def async_list_motion_sensor(self):
        ...
        for device in await self.async_get_devices():
            if device['product_type'] == "MotionSensor":  # MotionSensor
                motion_sensors.append(WyzeMotionSensor(
                    self,
                    device['mac'],
                    device['nickname'],
    ...
    async def async_do_request(self, url, payload):
        ...
        try:
            ...
            if payload["access_token"] not in self.__invalid_access_tokens:
                self.__invalid_access_tokens.append(payload["access_token"])
                await self.async_login()
